...
- **English-only errors**, no i18n—acceptable for this module as written; shipping worldwide would need real localization work.
...
- **Password max length**—easy add-on; v1 sticks to minimum strength checks only.
...
The reply was a bullet list per field; I folded it into the table below so the rules stay in one place and are easy to scan.
...
Beyond the original password checklist (8 chars + digit + special), I added upper/lowercase rules—slightly stricter, but it mirrors common password policies and pairs cleanly with the five-row SQL seed for passwords.
...
**Implementation notes:** regex objects and lengths live in **`VALIDATION_RULES`** once; **`validatePassword`** pulls checks via **`const { minLength, … } = VALIDATION_RULES.password`**; **`validate()`** aggregates per-field failures with **`Object.entries(...).flatMap`** into `{ field, message }` pairs for HTTP-shaped responses.

---

## 3. Database Schema & API

The table adds `field`, `is_active`, and timestamps—not just rule text—so rules can be turned off without deleting history and migrations stay stable as requirements evolve.
...
The repo runs **27** Jest cases (`npm test`). Naming in `validation.test.js` is set up so each test targets one behaviour and a failing case usually points straight at the rule that changed.
...
The suite grew to **27** cases after the initial draft—the extras caught non-string inputs, stacked password violations, and malformed phones.
...
README sits beside the module (`task_2/README.md`) so install, scripts, and layout are readable from the project folder alone.
...
I asked ChatGPT:
> "Please review this validation module like a quick technical check before handoff. Focus on security risks (for example SQL injection and XSS), expected performance (around 1000 requests/sec), and structural clarity in the code. Then suggest improvements I can realistically apply at this scope."
...
- **Parallel validator layout:** Same gate order across email/password/phone (see §2)—keeps the three validators easy to compare side by side.
